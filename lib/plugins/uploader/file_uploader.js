...
module.exports = (instance, files, forklift, filesystem, callback) => {
    ...
    Async.each(files, (file, eachCallback) => {
        ...
        if (filesystem && filesystem.path && filesystem.url) { // locale path.
            ...
            return Fse.move(localFilePath, Path.join(filesystem.path, uploadFolder), {overwrite: true}, (error) => {

                if (error) {
                    return eachCallback(error);
                }

                instance[file["schemaKey"]] = Path.join(filesystem.url, uploadFolder) + "?" + Date.now();
                return eachCallback();
            });
        }

        return forklift.upload(localFilePath, uploadFolder, {timestamp: true, ContentType: file["contentType"]}).then((url) => {
            
            instance[file["schemaKey"]] = `${url}?${Date.now()}`;
